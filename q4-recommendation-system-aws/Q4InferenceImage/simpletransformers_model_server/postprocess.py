...
def ripplenet_predict(model, user_id):
    ...
    mapping = 's3://zess-q3-recommendations/model_data_input/encoded_mapping.txt'

    with open(smart_open.smart_open(mapping), 'r') as f:
            mapping = json.load(f)
    #Load the model's parameters
    menu_items, (args, ripple_set, args.batch_size) = pickle.loads(s3.Bucket(bucket).Object(file_name).get()['Body'].read())
