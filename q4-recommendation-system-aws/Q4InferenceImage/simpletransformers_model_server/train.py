...
def train(args, data_info, show_loss):
    ...
    for step in range(args.n_epoch):
        ...
        test_auc, test_acc, test_scores = evaluation(
            model, test_data, (args, ripple_set, args.batch_size), flag=True
        )
...
def evaluation(model, data, params, flag):
    ...
    if flag:
        ...
        subprocess.run(['tar', '-czf', 'py_model.tar.gz', 'py_model.pkl'], stdout=subprocess.PIPE)

        s3.put_object(Bucket="zess-q3-recommendations",
                      Key='py_model.tar.gz', Body='py_model.tar.gz')
