...
class EatWellFeatures():
    ...
    def __init__(self) -> None:
        ...
        self.df = pd.read_csv("/tmp/ingredient_annotations.csv")
        # print(self.df)
        indexes = []
        ...
        self.calorie_deficit_params = {
            "user_energy_daily_intake": 2400,  # man
    ...
    def calorie_deficit(self, energy: float) -> float:
        """
        Calory restriction:
        ...
        meal_energy_threshold = self.calorie_deficit_params["user_energy_daily_intake"] / \
            self.calorie_deficit_params["meals_per_day"]
        user_meal_goal = self.calorie_deficit_params["user_energy_daily_goal"] / \
            self.calorie_deficit_params["meals_per_day"]
    ...
    def final_score_calculator(self, ingredients: list, num_of_servings: float, df: pd.DataFrame, verbose: bool) -> float:
        ...
        calorie_deficit_Score = self.calorie_deficit(energy)
        ...
        if sodium_potassium_score > 0.5:
            final_score = float((1.25 * carbs_protein_fat_ratio_score + 0.5 * sodium_intake_score + potassium_intake_score + 2 * calorie_deficit_Score + 2 * red_processed_meat_intake_score + fruit_vegetable_intake_score +
                                 1.5 * pufa_intake_score + 1.25 * mufa_intake_score + 1.5 * saturated_intake_score + 2.0 * trans_fats_intake + 0.25 * omega_ratio_score + 0.5 * fibre_intake_score + sodium_potassium_score) / 10)
        else:
            final_score = float((1.25 * carbs_protein_fat_ratio_score + sodium_intake_score + 0.5 * potassium_intake_score + 2 * calorie_deficit_Score + 2 * red_processed_meat_intake_score + fruit_vegetable_intake_score +
                                 1.5 * pufa_intake_score + 1.25 * mufa_intake_score + 1.5 * saturated_intake_score + 2.0 * trans_fats_intake + 0.25 * omega_ratio_score + 0.5 * fibre_intake_score + 1.6 * sodium_potassium_score) / 10)
        if verbose:
            print("carbs_protein_fat_ratio_score: ",
                  carbs_protein_fat_ratio_score)
            print("sodium_intake_score: ", sodium_intake_score)
            print("potassium_intake_score: ", potassium_intake_score)
            print("calorie_deficit_Score: ", calorie_deficit_Score)
            print("red_processed_meat_intake_score: ",
                  red_processed_meat_intake_score)
            print("fruit_vegetable_intake_score: ",
                  fruit_vegetable_intake_score)
            print("pufa_intake_score: ", pufa_intake_score)
            print("mufa_intake_score: ", mufa_intake_score)
            print("saturated_intake_score: ", saturated_intake_score)
            print("trans_fats_intake: ", trans_fats_intake)
            print("omega_ratio_score: ", omega_ratio_score)
            print("fibre_intake_score: ", fibre_intake_score)
            print("sodium_potassium_score: ", sodium_potassium_score)
            print("===========" * 3)
        return final_score, ingredients, energy, carbohydrate, protein, fat, salt, sodium, pufa, mufa, omega_3, omega_6, fibre, saturates, trans_fats, potassium
