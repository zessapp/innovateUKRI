...
class EatWellFeatures:
    def __init__(self) -> None:
        ...
        #print(os.getcwd())
        self.df = pd.read_csv(f"../eat-well/eat_well/data/ingredient_annotations.csv")

        indexes = []
        for row in self.df.iterrows():
            if row[1]["Not food/drink"] != "checked" and not math.isnan(
                row[1]["Energy (Kcal)"]
            ):
                indexes.append(row[0])
        self.annotated_df = self.df.iloc[indexes].reset_index(drop=True)

        self.carbs_protein_fat_ratio_params = {
        ...
        # Ideal intakes
        self.potassium_intake_params = {"daily_potassium_intake": 3, "meals_per_day": 3}

        self.sodium_potassium_intake_params = {"ratio": np.round(float(1 / 3), 3)}

        self.meals_per_day_params = {"meals_per_day": 3}
        # TODO
        ...
        self.pufa_intake_params = {"pufa_fats_ratio": 0.5}

        self.mufa_intake_params = {"mufa_fats_ratio": 0.25}
        ...
        self.trans_fats_intake_params = {"transfats_fats_ratio": 0}

        self.omega_ratio_params = {"omega_3_to_omega_6_ratio": 2}
    ...
    def carbs_protein_fat_ratio(
        self, energy: float, carbohydrate: float, protein: float, fat: float
    ) -> float:
        """
        ...
        carbs_energy_lower_bound = (
            self.carbs_protein_fat_ratio_params["carbohydrates_lower_bound"] * energy
        )
        carbs_energy_upper_bound = (
            self.carbs_protein_fat_ratio_params["carbohydrates_upper_bound"] * energy
        )

        protein_energy_lower_bound = (
            self.carbs_protein_fat_ratio_params["protein_lower_bound"] * energy
        )
        protein_energy_upper_bound = (
            self.carbs_protein_fat_ratio_params["protein_upper_bound"] * energy
        )

        fat_energy_lower_bound = (
            self.carbs_protein_fat_ratio_params["fat_lower_bound"] * energy
        )
        fat_energy_upper_bound = (
            self.carbs_protein_fat_ratio_params["fat_upper_bound"] * energy
        )
        ...
        if (
            carbohydrate >= carbs_weight_lower_bound
            and carbohydrate <= carbs_weight_upper_bound
        ):
            carbs_score = 1
            ...
            carbs_score = (
                1 - (carbohydrate - carbs_weight_upper_bound) / carbs_weight_upper_bound
            )

        if (
            protein >= protein_weight_lower_bound
            and protein <= protein_weight_upper_bound
        ):
            protein_score = 1
            ...
            protein_score = (
                1 - (protein - protein_weight_upper_bound) / protein_weight_upper_bound
            )
        ...
        if fat >= fat_weight_lower_bound and fat <= fat_weight_upper_bound:
            ...
            fat_score = 1 - (fat - fat_weight_upper_bound) / fat_weight_upper_bound

        # Alter the weighting based on the user profile
        score = (
            self.carbs_protein_fat_ratio_params["carbs_score_weight"] * carbs_score
            + self.carbs_protein_fat_ratio_params["protein_score_weight"]
            * protein_score
    ...
    def carbs_intake_ratio(self, energy: float, carbohydrate: float) -> float:
        ...
        # 4kcal per g carbs
        carbohydrate_energy = (
            self.carbs_protein_fat_ratio_params["kcal_per_g_carbs"] * carbohydrate
        )
    ...
    def sodium_intake(self, salt: float, sodium: float) -> float:
        ...
        salt_upper_bound_per_meal = (
            self.sodium_intake_params["daily_salt_intake"]
        ...
        if salt > 0:
            if salt < salt_upper_bound_per_meal:
                ...
                salt_score = (
                    1 - (salt - salt_upper_bound_per_meal) / salt_upper_bound_per_meal
                )
        elif salt == 0 and sodium > 0:
            sodium_upper_bound_per_meal = (
                self.sodium_intake_params["daily_sodium_intake"]
                / self.sodium_intake_params["meals_per_day"]
            )
    ...
    def potassium_intake(self, potassium: float) -> float:
        ...
        if potassium > 0:
            potassium_upper_bound_per_meal = (
                self.potassium_intake_params["daily_potassium_intake"]
                / self.potassium_intake_params["meals_per_day"]
            )
    ...
    def sodium_potassium_intake(self, sodium: float, potassium: float):
        ...
        if potassium > 0:
            ...
            if sodium_potassium_ratio <= self.sodium_potassium_intake_params["ratio"]:
                sodium_potassium_ratio_score = 1 - 0.5 * (
                    abs(
    ...
    def calorie_deficit(
        ...
        user_meal_goal = (
            calorie_intake - (calorie_intake * (energy_percent / 100)) / meals_per_day
        )
    ...
    def red_processed_meat_intake(self, ingredients: list) -> float:
        ...
        for ingredient in ingredients:
            ...
            try:
                if (
                    list(
                        self.annotated_df[
                            self.annotated_df["Ingredient (as per 100 g)"] == ingredient
                        ]["Red/Processed Meat"]
                    )[0]
                    == "checked"
                ):
        ...
        meat_intake_per_meal = np.round(
            self.red_processed_meat_intake_params["red_processed_meat_per_day"]
            / self.red_processed_meat_intake_params["meals_per_day"]
        )
        if red_processed_meat_weight <= meat_intake_per_meal:
            score = 1
        elif (
            red_processed_meat_weight > meat_intake_per_meal
            and red_processed_meat_weight <= 1.5 * meat_intake_per_meal
        ):
            score = 0.5
        else:
            score = -0.5
    ...
    def fruit_vegetable_intake(self, ingredients: list) -> float:
        ...
        # At least 5 fruits or vegetables a day, i.e. 2 veg/fruits per meal.
        if veg_fruit_count >= np.round(
            self.fruit_vegetable_intake_params["fruit_veg_per_day"]
            / self.fruit_vegetable_intake_params["meals_per_day"]
        ):
            score = 1
    ...
    def pufa_intake(self, fat: float, pufa: float) -> float:
        ...
        if fat == 0:
            ...
            if pufa_fat_ratio >= self.pufa_intake_params["pufa_fats_ratio"]:
                score = 1
            elif pufa_fat_ratio < self.pufa_intake_params[
                "pufa_fats_ratio"
            ] and pufa_fat_ratio >= (0.7 * self.pufa_intake_params["pufa_fats_ratio"]):
                score = 0.5
            else:
                score = 0
    ...
    def mufa_intake(self, fat: float, mufa: float) -> float:
        ...
        if fat == 0:
            ...
            if mufa_fat_ratio >= self.mufa_intake_params["mufa_fats_ratio"]:
                score = 1
            elif mufa_fat_ratio < self.mufa_intake_params[
                "mufa_fats_ratio"
            ] and mufa_fat_ratio >= (0.7 * self.mufa_intake_params["mufa_fats_ratio"]):
                score = 0.5
            else:
                score = 0
    ...
    def saturated_intake(self, fat: float, saturated: float) -> float:
        ...
        if fat == 0:
            ...
            if (
                saturated_fat_ratio
                >= self.saturated_intake_params["saturatedfats_fats_ratio_upper_bound"]
            ):
                score = -0.5
            elif (
                saturated_fat_ratio
                < self.saturated_intake_params["saturatedfats_fats_ratio_upper_bound"]
                and saturated_fat_ratio
                >= self.saturated_intake_params["saturatedfats_fats_ratio_lower_bound"]
            ):
                score = 1
            else:
                score = 0.5
    ...
    def trans_fats_intake(self, fat: float, trans_fats: float) -> float:
        ...
        if fat == 0:
            ...
            if trans_fats_fat_ratio >= 0.01:
                ...
                trans_fats_fat_ratio < 0.01
                and trans_fats_fat_ratio
                > self.trans_fats_intake_params["transfats_fats_ratio"]
            ):
                score = 0.5
            elif (
                trans_fats_fat_ratio
                == self.trans_fats_intake_params["transfats_fats_ratio"]
            ):
                score = 1
    ...
    def omega_ratio(self, omega_3: float, omega_6: float) -> float:
        ...
        if omega_6 == 0:
            ...
            if ratio >= self.omega_ratio_params["omega_3_to_omega_6_ratio"]:
                score = 1
            elif ratio < self.omega_ratio_params[
                "omega_3_to_omega_6_ratio"
            ] and ratio >= (0.5 * self.omega_ratio_params["omega_3_to_omega_6_ratio"]):
                score = 0.5
    ...
    def fibre_intake(self, fibre: float) -> float:
        ...
        """
        fibre_intake_per_meal = (
            self.fibre_intake_params["daily_fibre_intake"]
    ...
    def polyphenol_intake(self, polyphenol: float) -> float:
        ...
        # from mg to g
        polyphenol = 0.01 * self.polyphenol_intake_params["daily_polyphenol_intake"]
        recommended_polyphenol_intake_per_meal = (
            polyphenol / self.polyphenol_intake_params["meals_per_day"]
        )
        if polyphenol >= recommended_polyphenol_intake_per_meal:
            score = 1
        elif polyphenol < recommended_polyphenol_intake_per_meal and polyphenol >= (
            recommended_polyphenol_intake_per_meal / 2
        ):
            score = 0.5
        else:
            score = 0
    ...
    def fermented_food_intake(self, ingredients: list) -> float:
        ...
        fermented_food_per_meal = np.round(
            self.fermented_food_intake_params["daily_fermented_food_intake"]
            / self.fermented_food_intake_params["meals_per_day"]
        )
    ...
    def alcohol_intake(self, alcohol: float) -> float:
        ...
        """
        recommended_alcohol_intake_per_meal = (
            self.alcohol_intake_params["daily_alcohol_intake"]
            / self.alcohol_intake_params["meals_per_day"]
        )
        if alcohol <= recommended_alcohol_intake_per_meal:
            score = 1
        elif alcohol >= recommended_alcohol_intake_per_meal and alcohol <= (
            1.3 * recommended_alcohol_intake_per_meal
        ):
            score = 0.5
        else:
            score = 0
    ...
    def final_score_calculator(
        ...
        carbohydrate = self.request_nutrient("carbohydrate", df) / num_of_servings
        ...
        carbs_intake_ratio_score = self.carbs_intake_ratio(energy, carbohydrate)
        sodium_intake_score = self.sodium_intake(salt, sodium)
        potassium_intake_score = self.potassium_intake(potassium)
        calorie_deficit_Score = self.calorie_deficit(
            energy, calorie_intake, energy_percent, meals_per_day
        )
        red_processed_meat_intake_score = self.red_processed_meat_intake(ingredients)
        fruit_vegetable_intake_score = self.fruit_vegetable_intake(ingredients)
        ...
        sodium_potassium_score = self.sodium_potassium_intake(sodium, potassium)
        contains_red_meat = self.eliminate_red_meat(ingredients)
        sugar_score = self.sugar_intake(sugar, meals_per_day)

        nutrients = [
            carbs_protein_fat_ratio_score,
            carbs_intake_ratio_score,
            sodium_intake_score,
            potassium_intake_score,
            calorie_deficit_Score,
            red_processed_meat_intake_score,
            fruit_vegetable_intake_score,
            pufa_intake_score,
            mufa_intake_score,
            saturated_intake_score,
            trans_fats_intake,
            omega_ratio_score,
            fibre_intake_score,
            sodium_potassium_score,
            sugar_score,
            omega_3_score,
        ]
