...
def calc_bmr(
    sex: str, weight: float, weight_unit: str, height: float, height_unit: str, age: int
) -> float:
...
def tdee(
    ...
    activity_reduction = 0.98

    if activity_level.lower() == "sedentary":
        activity = 1.2
    if activity_level.lower() == "light":
        activity = 1.375
    if activity_level.lower() == "moderate":
        activity = 1.55
    if activity_level.lower() == "very":
        activity = 1.725
    if activity_level.lower() == "extremely":
        activity = 1.9

    return bmr * (activity * activity_reduction)
