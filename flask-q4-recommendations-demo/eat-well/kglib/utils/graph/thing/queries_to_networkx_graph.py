...
def concept_dict_from_concept_map(concept_map):
    ...
    """
    return {
        variable: build_thing(typedb_concept)
        for variable, typedb_concept in concept_map.map().items()
    }
...
def build_graph_from_queries(
    ...
    for query, sampler, variable_graph in query_sampler_variable_graph_tuples:
        ...
        concept_dicts = [
            concept_dict_from_concept_map(concept_map) for concept_map in concept_maps
        ]
        ...
        for concept_dict in concept_dicts:
            try:
                answer_concept_graphs.append(
                    concept_dict_converter(concept_dict, variable_graph)
                )
            except ValueError as e:
                raise ValueError(
                    str(e) + f'Encountered processing query:\n "{query}"')

        if len(answer_concept_graphs) > 1:
            query_concept_graph = combine_graphs_single_pass(
                answer_concept_graphs)
