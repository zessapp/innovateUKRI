...
if __name__ == "__main__":
    ...
    df_menus.rename(
        columns={"main": "starters & side dishes",
                 "starters & side dishes": "main"},
        inplace=True,
    )
    ...
    # Replace ingredients with the simplified ingredients that were crowdsourced and validated by Zess annotators.

    df_filtered_ingredients_mapper = pd.read_csv(
        "data/filtered_ingredients_google_sheet.csv")
    for pos, info in enumerate(total_info):
        for idx, i in enumerate(info[3]):
            try:
                total_info[pos][3][idx] = df_filtered_ingredients_mapper[df_filtered_ingredients_mapper["Input Ingredient"] == i]["Final Ingredient"].tolist()[
                    0]
            except:
                pass

    # Remove empty/NaN values in ingredients list. NaN values means that the original ingridient value was not an actual ingredient.
    for pos, info in enumerate(total_info):
        total_info[pos][3] = [x for x in info[3] if str(x) != 'nan']

    # Generate a corpus of all menu items' ingredients to calculate tf-idf score for the ingredients of every menu item
    corpus = []
    for idx, info in enumerate(total_info):
        sub_corpus = ""
        for ing in info[3]:
            temp = ing.replace("  ", "_")
            temp = temp.replace(" ", "_")
    ...
    for recipe_index in range(len(corpus)):
        ...
        for idx, score in enumerate(df["TF-IDF"]):
            recipe_score_dict[df.index[idx].replace(
                "_", " ")] = round(score, 2)
