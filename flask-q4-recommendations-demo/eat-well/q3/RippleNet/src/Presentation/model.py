...
class RippleNet(nn.Module):
    def __init__(self, args, n_entity, n_relation):
        ...
        self.entity_emb = nn.Embedding(
            self.n_entity + self.n_relation, self.dim)
        ...
        for hop in range(self.n_hop):
            ...
            hRt = torch.squeeze(
                torch.matmul(torch.matmul(
                    h_expanded, r_emb_list[hop]), t_expanded)
            )
    ...
    def _key_addressing(self, h_emb_list, r_emb_list, t_emb_list, item_embeddings):
        ...
        for hop in range(self.n_hop):
            ...
            # [batch_size, n_memory]
            probs_normalized = F.softmin(probs, dim=1)
    ...
    def evaluate(self, items, labels, memories_h, memories_r, memories_t, flag):
        if flag:
            return_dict = self.forward(
                items, labels, memories_h, memories_r, memories_t, flag)
