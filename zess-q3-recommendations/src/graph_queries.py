...
def create_concept_graphs(
    ...
    graphs = []
    query, node_vars, relation_vars = create_queries(query_all=query_all, limit=limit)
...
def build_graph(query: str, node_vars: list, relation_vars: list) -> QueryGraphAZ:
    ...
    for i, relation in enumerate(relation_vars):
        ...
        if "to" in relation:
            ...
            graph = graph.add_role_edge(relation, node_vars[i], "cuisine-contains")
