...
class RippleNet(nn.Module):
    def __init__(self, args, n_entity, n_relation):
        ...
        self.entity_emb = nn.Embedding(self.n_entity + self.n_relation, self.dim)
    ...
    def forward(
        ...
        #     print(idx, i, len(items))
        #     print(self.entity_emb(torch.LongTensor([i])))
        item_embeddings = self.entity_emb(items)
        # print("3", item_embeddings[0].shape)
        h_emb_list = []
        r_emb_list = []
        t_emb_list = []
        for i in range(self.n_hop):
            ...
            # for i in memories_h[i]:
            #     print("1", i)
            #     for j in i:
            #         print(j)
            #         print(self.entity_emb(torch.LongTensor([j])))
            #         print()
            # time.sleep(1000)
