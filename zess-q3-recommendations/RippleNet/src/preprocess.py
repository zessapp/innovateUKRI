...
RATING_FILE_NAME = dict(
    {"movie": "ratings.dat", "book": "BX-Book-Ratings.csv", "news": "ratings.txt"}
)
...
def convert_kg():
    ...
    if DATASET == "movie":
        files.append(
            open("../data/" + DATASET + "/kg_part1_rehashed.txt", encoding="utf-8")
        )
        files.append(
            open("../data/" + DATASET + "/kg_part2_rehashed.txt", encoding="utf-8")
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--dataset", type=str, default="movie", help="which dataset to preprocess"
    )
