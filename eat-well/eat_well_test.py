...
s3#Generate plots per nutrient
...
eat_well_ratings = {
    ...
    "all_lunch_dishes": ['Tequila Laced Gazpacho Cocktails with Grilled Shrimp',
...
rbo([i.lower() for i in ground_truth_ratings["all_lunch_dishes"]], [i.lower() for i in eat_well_ratings["all_lunch_dishes"]])

import json


df_main.set_index('allergens_free_menu_items', inplace=True)
print(json.dumps(json.loads(df_main.to_json(orient='index')), indent=2))
