...
def predict_ingredients(ds, predictor, ingredient_raw_list, ingredient_prep_list):
    ...
    for file_name in [file for file in os.listdir(f'{os.getcwd()}/data/{ds}/input/') if file.endswith('.json')]:
        ...
        for key in  data.keys():
            ...
            prepared_full, raw_full = [], []
            # Main data
            current_recipe_d['ingredients'] = str(data[key]['recipe']['ingredients'].replace("'", '').split(', '))
                ...
                for prediction_value in prediction_result:
                    ...
                    if raw == '' and prep == '':
                        ...
                        if len(prep_list) > 0:
                            ...
                            prepared_full.append(prep_list)

                        unit_list_l.append((', ').join(unit_list).replace(',', ''))
                        measure_list_l.append((', ').join(measure_list).replace(',', ''))

                    current_recipe_d['raw ingredients joined'] = list(filter(None, current_raw))
                    current_recipe_d['prepared ingredients joined'] = list(filter(None, current_prep))
                    if prepared_full:
                        current_recipe_d['prepared ingredients list'] = list(filter(None, prepared_full))
                    if raw_full:
                        current_recipe_d['raw ingredients list'] = list(filter(None, raw_full))

                    measure = list(filter(None, measure_list_l))
                    if len(measure) == 0:
                        measure.append('Not recorded')
                    unit = list(filter(None, unit_list_l))
                    current_recipe_d['measures'] = dict(zip(measure, unit))
                    recipe_d[recipe_name] = current_recipe_d
            except:
                print(f'failed prediction for {predict_str}')
                pass
...
airtable_raw = Airtable(base_airtable_raw, api_key_airtable)
airtable_prep = Airtable(base_airtable_prep, api_key_airtable)
...
with open(f'{os.getcwd()}/datasources.txt', 'r') as d:
    ...
    for ds in datasources:
        ...
        test = predict_ingredients(ds=ds, predictor=predictor, ingredient_raw_list=ingredient_raw_list,
